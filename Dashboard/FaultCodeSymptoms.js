...
const  FaultCodeSymptoms = async(req,res) =>  {
    try {
        ...
        const query = `
        SELECT 
        f.* FROM  my_fault_code_symptoms f
        ${whereClause}
        ORDER BY f.make ASC
        ...
        const countQuery = `
         SELECT 
        COUNT(*) AS total FROM  my_fault_code_symptoms f
        ${whereClause}
        `
        ...
        console.error("Error fetching fault codes symptoms", error);
...
module.exports =  FaultCodeSymptoms;
