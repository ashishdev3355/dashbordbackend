...
const  FaultCodeSolutions = async(req,res) =>  {
    try {
        ...
        const query = `
        SELECT 
        f.* FROM my_fault_code_solutions f
        ${whereClause}
        ORDER BY f.make ASC
        ...
        const countQuery = `
         SELECT 
        COUNT(*) AS total FROM my_fault_code_solutions f
        ${whereClause}
        `
        ...
        console.error("Error fetching fault codes solutions", error);
...
module.exports =  FaultCodeSolutions;
