  ...
  try {
    ...
    const query = `
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        make_group_id = EXCLUDED.make_group_id
    `;
...
router.post('/', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const sheetName = req.body.sheetName ? req.body.sheetName.trim() : null;
    ...
    const companyRes = await pool.query(
      `SELECT id FROM car_companies WHERE name = $1 ORDER BY id LIMIT 1`,
      [sheetName]
    );
